-- Add a computed column for normalized chain names
ALTER TABLE public.vip_sales ADD COLUMN normalized_chain TEXT;

-- Create a function to normalize chain names
...
-- Update all existing records with normalized chain names
UPDATE public.vip_sales 
SET normalized_chain = normalize_chain_name("Retail Accounts");

-- Create an index for better performance
CREATE INDEX idx_vip_sales_normalized_chain ON public.vip_sales(normalized_chain);

-- Create a trigger to automatically update normalized_chain when Retail Accounts changes
