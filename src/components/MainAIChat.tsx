...
const mockData = {
  ...
  accountsData: [{
    name: "Woolworths Metro CBD",
    region: "NSW",
    chain: "Woolworths",
    sales: [45000, 52000, 38000],
    status: "churn-risk"
  }, {
    name: "Coles Express Kings Cross",
    region: "NSW",
    chain: "Coles",
    sales: [32000, 29000, 0],
    status: "dropped"
  }, {
    name: "IGA Bondi Junction",
    region: "NSW",
    chain: "IGA",
    sales: [18000, 22000, 28000],
    status: "growing"
  }, {
    name: "7-Eleven Central",
    region: "NSW",
    chain: "7-Eleven",
    sales: [15000, 18000, 24000],
    status: "growing"
  }, {
    name: "Woolworths Parramatta",
    region: "NSW",
    chain: "Woolworths",
    sales: [67000, 71000, 65000],
    status: "stable"
  }]
};
...
export const MainAIChat = () => {
  ...
  const quickQuestions = ["Who are my churn-risk accounts?", "Show me chain breakdown", "Top 5 accounts in NSW?", "Which stores dropped off in July?"];
        ...
        <Card className="shadow-card border-0">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {mockData.chainData.map((chain, index) => <div key={index} className="p-4 rounded-lg border bg-gradient-to-r from-card to-card/50 hover:shadow-lg transition-all">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-semibold text-foreground text-lg">{chain.name}</h4>
                    <Badge variant={chain.status === "growing" ? "default" : "destructive"} className={chain.status === "growing" ? "bg-success text-success-foreground" : "bg-destructive text-destructive-foreground"}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="pt-2">
                      ...
                      <div className="flex gap-1">
                        {chain.totalSales.map((sales, monthIndex) => <div key={monthIndex} className="flex-1 bg-secondary rounded-sm overflow-hidden">
                            <div className={`h-2 rounded-sm ${chain.status === "growing" ? "bg-success" : "bg-destructive"}`} style={{
          ...
          <Card className="shadow-card border-0">
            ...
            <CardContent>
              <div className="space-y-4">
                {mockData.accountsData.map((account, index) => <div key={index} className="flex items-center justify-between p-4 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors">
                    ...
                    <div className="text-right">
                      <span className={`font-medium ${account.status === "growing" ? "text-success" : account.status === "churn-risk" || account.status === "dropped" ? "text-destructive" : "text-muted-foreground"}`}>
